...
public class Boggle extends JFrame implements ActionListener {
	private static final long serialVersionUID = 1L; // idk why it's here
	// game variables
	...
	private void showMenu() { // construct menu
		...
		JLabel boggleTitle = new JLabel("b o g g l e", JLabel.CENTER);
		...
		boggleTitle.setFont(new Font("Calibri", Font.PLAIN, 130));
		...
		Font baseFont = new Font("Calibri", Font.PLAIN, 15); // used for everything
		...
		singlePlayer.setFont(new Font("Calibri", Font.PLAIN, 30));
		...
		multiPlayer.setFont(new Font("Calibri", Font.PLAIN, 30));
	...
	private void constructGame() { // construct boggle game
		...
		for (int r = 0; r < dices.length; r++) {
			for (int c = 0; c < dices[0].length; c++) {
				...
				cells[r][c].setFont(new Font("Calibri", Font.BOLD, 20));
				grid.add(cells[r][c]);
